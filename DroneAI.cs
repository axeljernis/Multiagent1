...
public class DroneAI : MonoBehaviour
{
    ...
    private float[] dronesize = new float[] { 3, 3 };
    ...
    terrain_manager = terrain_manager_game_object.GetComponent<TerrainManager>();
        ...
        newMap = InflateObstacles(newMap, dronesize, tinfo);

        my_path = this.SearchPath(start_pos, goal_pos, newMap, tinfo);

        //my_path[0] = start_pos;
        //my_path[1] = (m_Drone.transform.forward + start_pos);
        ...
        {
            my_path[i] = CheckIfWaypointcollidesAndAdjust(my_path[i], newMap, tinfo, dronesize);
    ...
    private List<Vector3> SearchPath(Vector3 start_pos, Vector3 goal_pos, float[,] newMap, TerrainInfo tinfo)
    {
        ...
        Debug.Log(m_Drone.transform.forward.x + " " + m_Drone.transform.forward.y);
        int counter = 0;
    ...
    private Vector3 CheckIfWaypointcollidesAndAdjust(Vector3 wp, float[,] newMap, TerrainInfo tinfo, float[] dronesize)
    {
        ...
        double xfactor = ((dronesize[0] / xstep)); //number of steps in xdirection corresponding to dronesize
        double zfactor = ((dronesize[1] / zstep)); //number of steps in zdiretion corresponding to dronesize
        Vector3 newVect = wp;
        float[] sqrtdronesize = new float[] { (float)Math.Sqrt((double)dronesize[0]), (float)Math.Sqrt((double)dronesize[1]) };
    ...
    private float[,] InflateObstacles(float[,] newMap, float[] dronesize, TerrainInfo tinfo)
    {
        float[] size = tinfo.getSize();
        float xstep = (size[0]) / ((float)newMap.GetLength(0)); //size of step in x direction
        float zstep = (size[1]) / ((float)newMap.GetLength(1)); //size of step in z direction
            ...
            for (int j = 1; j < newMap.GetLength(1); j++)
            {
                ...
                for (int i = 1; i < ((int)xfactor+1); i++)
                {
                    ...
                    else
                    {
                        ...
                        else if (newMap[k - i-1, j + i+1] > 0.8)
                        {
                            newMap[k, j] = 0.6f;
                            break;
                        }
                        ...
                        else if (newMap[k - i - 2, j] > 0.8)
                        {
                            newMap[k, j] = 0.6f;
                            break;
                        }
                        else if (newMap[k + i+1, j] > 0.8)
                        {
                            newMap[k, j] = 0.6f;
                            break;
                        }
                        else if (newMap[k, j+i+2] > 0.8)
                        {
                            newMap[k, j] = 0.6f;
                            break;
                        }
    ...
    {
        Vector3 dronePos = m_Drone.transform.position;
        Vector3 velocity = (oldPos - dronePos) / Time.fixedDeltaTime;
        Vector3 directionforward = m_Drone.transform.forward;
        Vector3 toWaypoint = my_path[wp] - dronePos + new Vector3(0f,3f,0f);
        oldPos = dronePos;
        Quaternion carRot = m_Drone.transform.rotation;
        ...
        float distWP = PathSegment.getDistToWP(my_path[wp], dronePos.z, dronePos.x);
        float distRay = PathSegment.getDistToRay(pathAngles[wp], my_path[wp], dronePos.z, dronePos.x);
        float biasRay = 0;
        if (distRay > 0.3f)
        {
            //biasRay = 20;
        ...
        else
        {
            ...
            float angletowp = PathSegment.getAngleForMiddle(dronePos, my_path[wp]);
            float dirwp = 1;
            ...
            float dir = 1;
            //float angleWP = -PathSegment.getAngleForMiddle(dronePos, my_path[wp]);
            ...
            if (PathSegment.getDistToWP(my_path[wp], dronePos.z, dronePos.x) < 3f)
            {
                wp++;
                towards = 1;
                lastDistance = 100000;
            ...
            if (PathSegment.getDistToWP(my_path[wp], dronePos.z, dronePos.x) < 3f && distWP > lastDistanceCar)
            {
                wp++;
                towards = 1;
                lastDistance = 100000;
            ...
            for (int k = wp; k < 20 + wp; k++)
            {
                ...
                if (dyn < (float)Math.Abs((double)pathAnglesDiff[k]) / (PathSegment.getDistToWP(my_path[k], dronePos.z, dronePos.x) + 6f))
                {
                    dyn = (float)Math.Abs((double)pathAnglesDiff[k]) / (PathSegment.getDistToWP(my_path[k], dronePos.z, dronePos.x) + 6f);
                }
            }
            ...
            {
                toWaypoint = 0*toWaypoint;
            }
            Vector3 diffvect = 102*toWaypoint.normalized + 99*velocity.normalized;
            Debug.Log("angle" + Vector3.Angle(velocity, -toWaypoint.normalized));
            Debug.Log(velocity);
            Debug.Log(velocity.magnitude);
            Debug.Log(toWaypoint);
            velocity = velocity.normalized;
            toWaypoint = toWaypoint.normalized;
            diffvect = diffvect.normalized;


            Debug.Log(diffvect.x +" wp " + diffvect.z);




            m_Drone.Move(diffvect.x, diffvect.z);
